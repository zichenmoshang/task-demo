...
var AddHtmlTemplate = function () {
	function AddHtmlTemplate(strategy, data) {
		(0, _classCallCheck3.default)(this, AddHtmlTemplate);

		this[strategy](data);
	}

	(0, _createClass3.default)(AddHtmlTemplate, [{
		key: 'tab',
		value: function tab(data) {
			var dom = document.getElementById('tabs');
			var tpl = [];
		...
		value: function list(data) {
			...
			try {
				for (var _iterator = (0, _getIterator3.default)(data), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
					...
					try {
						for (var _iterator2 = (0, _getIterator3.default)(group), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
	...
	(0, _createClass3.default)(Scroll, [{
		...
		value: function register() {
			...
			this.dom.addEventListener('touchstart', function (e) {
				_this.opsition.startX = e.touches[0].pageX;
				_this.opsition.startY = e.touches[0].pageY;
				var matchArr = _this.dom.style.transform.match(/(-?\d+\.?\d*)(?=px)/g);
				_this.opsition.left = Number(matchArr[0]);
				_this.opsition.top = Number(matchArr[1]);
			});
			this.dom.addEventListener('touchmove', function (e) {
				if (!_this.ticking) {
					_this.opsition.endX = e.touches[0].pageX;
					_this.opsition.endY = e.touches[0].pageY;
					var deltaX = _this.opsition.left + _this.opsition.endX - _this.opsition.startX;
					var deltaY = _this.opsition.top + _this.opsition.endY - _this.opsition.startY;
					_this.raf = throttle(function () {
						_this.ticking = false;
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(36), __webpack_require__(77)))
		...
		return _regenerator2.default.wrap(function _callee$(_context) {
			while (1) {
				switch (_context.prev = _context.next) {
					case 0:
						_context.next = 2;
						return _tools2.default.domLoaded();

					case 2:
						_context.next = 4;
						return _tools2.default.getJson('js/mockData.json');
...
var _tools = __webpack_require__(37);
...
function cartCacheFn() {
	var listInput = document.querySelectorAll('.list-input'),
...
function inputChange(i, price, cache, value) {
	...
	var goodsprice = Number(cache.goodsPrice.innerText) + delta * Number(price.replace(/\$/g, ''));
	if (goodsnum === 0) {
		cache.cartDom.style.display = 'none';
	} else {
		cache.goodsNum.innerText = goodsnum;
...
asyncInit().then(function (data) {
	//load template
	new _tools2.default.AddHtmlTemplate('tab', data.tabItems);
	new _tools2.default.AddHtmlTemplate('list', data.lists);
	//get cache
	var tabsCache = tabsCacheFn();
	var listCache = listCacheFn(data.lists);
	var cartCache = cartCacheFn();
	//current index
	var lastIndex = 0;
	//bind scroll event
	new _tools2.default.Scroll('tabs', function (pos) {
		if (pos.deltaX <= tabsCache.maxWidth && pos.deltaX >= tabsCache.minWidth) {
			...
			var i = Math.abs(parseInt((pos.deltaX - tabsCache.middleWidth) / tabsCache.itemWidth));
			if (lastIndex !== i) {
	...
	new _tools2.default.Scroll('list', function (pos) {
		if (pos.deltaY <= listCache.maxHeight && pos.deltaY >= listCache.minHeight) {
			...
			var i = Math.abs(parseInt(pos.deltaY / listCache.listHeight));
			var tmpArr = listCache.lenArray.slice();
			tmpArr.push(i);
	...
	//delegate event
	listCache.dom.addEventListener('click', function (e) {
		var dom = e.target;
		switch (dom.tagName) {
			case 'I':
			case 'A':
				var group = dom.dataset.group;
				var num = dom.dataset.num - listCache.lenArray[group];
	...
	//delegate event
	listCache.dom.addEventListener('keydown', function (e) {
	...
	//delegate event
	listCache.dom.addEventListener('keyup', function (e) {
		var dom = e.target;
